...
func (l *Light) Verify(block Block, shareDiff *big.Int) (bool, bool) {
	// TODO: do ethash_quick_verify before getCache in order
	// to prevent DOS attacks.
	blockNum := block.NumberU64()
	if blockNum >= epochLength*2048 {
		log.Debug(fmt.Sprintf("block number %d too high, limit is %d", block.NumberU64(), epochLength*2048))
		return false, false
	}
	...
	*/
	if difficulty.Cmp(common.Big0) == 0 {
		...
		return shareDiff.Cmp(common.Big0) == 0, false
	}
	...
	// add share target to block
	shareTarget := new(big.Int).Div(maxUint256, shareDiff)
	return result.Big().Cmp(shareTarget) <= 0, result.Big().Cmp(target) <= 0
}
